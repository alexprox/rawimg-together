...
function getGames()
{
	$.ajax({
		...
		$('.games .list').html('');
		if(data.length == 0)
			$('.games .list').html('<h1>Доступных игр нет</h1>');
		else
		{
			var e = $('<button class="btn btn-large btn-block btn-info game">'+data[k]+'</button>');
				e.on('click', selectGame);
				e.appendTo('.games .list');
		}
	}).done(function( msg ){
	});
}
...
function getUsers()
{
	$.ajax({
		...
		$('.users .list').html('');
		if(data.length == 0)
			$('.users .list').html('<h1>Доступных игроков нет</h1>');
		else
		{
			for(k in data)
			{
				var e = $('<button class="btn btn-large btn-block btn-info user">'+data[k]+'</button>');
				e.on('click', selectUser);
				e.appendTo('.users .list');
			}
		}
	}).done(function( msg ){
	});
